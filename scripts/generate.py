...
PROJECTS: dict[str, list[Project]] = {
    "progressive-disclosure": [
        ...
        Project("DESIGN.md", "google-labs-code/design.md",
                "Format spec for describing a visual identity to coding agents: a structured DESIGN.md file agents load in layers so brand and UI conventions travel with the repo instead of being re-explained per prompt—the design-system sibling to agents.md.",
    ...
    "coding-agent-products": [
        ...
        Project("Grok CLI", "superagent-ai/grok-cli",
                "Terminal coding agent for the Grok API from Superagent. The **harness** is a straightforward tool-call loop; the CLI is the shell.",
                "slightly complex (terminal agent, single-provider)", oss="❓"),
        Project("Ante", "AntigmaLabs/ante",
                "Self-contained Rust terminal coding agent pitched as a dependency-free, model-unconstrained alternative to Claude Code or Codex. The **harness** is a standalone core with no upstream ties.",
                "slightly complex (terminal agent, zero deps)", oss="❓", labels=["rust"]),
        Project("Antigravity CLI", "google-antigravity/antigravity-cli",
                "Google's official terminal front end for the Antigravity agent **harness**: brings the reasoning/execution/orchestration loop from Google's agentic IDE into the terminal, parallel to Gemini CLI but for the Antigravity product line.",
                "slightly complex (official CLI, IDE harness ported to terminal)", oss="❓"),
        Project("agent-teams-ai", "777genius/agent-teams-ai",
                "Kanban fleet manager where agents message each other and review each other's work across 75+ LLM providers (Codex, Claude, OpenCode, Cursor, Copilot, and more). The **harness** contribution is the task-board/inter-agent-messaging layer, not a new agent loop.",
                "complex (multi-agent kanban, 75+ providers — product suite)", oss="❓"),
        Project("open-cowork", "OpenCoworkAI/open-cowork",
                "Open-source desktop **harness** positioned as a self-hosted alternative to Claude Cowork: one-click Claude Code/MCP/Skills install with sandbox isolation, multi-model support, and Feishu/Slack integration.",
                "complex (desktop multi-model workspace — product suite)", oss="❓"),
    ],
    "coding-harness-configs": [
        ...
        Project("DeepCode", "HKUDS/DeepCode",
                "HKUDS's open agentic-coding **harness**: explicit loop engineering and multi-agent orchestration for coding tasks, from the same lab behind this list's OpenHarness and nanobot entries.",
                "slightly complex (loop engineering, multi-agent)", oss="❓"),
    ],
    "personal-agent-runtimes": [
        ...
        Project("agenticSeek", "Fosowl/agenticSeek",
                "Fully local, API-free autonomous agent **harness**: one loop that thinks, browses the web, and writes code against local models—no cloud calls, no subscription.",
                "slightly complex (local-only autonomous loop)", oss="❓"),
    ],
    "frameworks": [
    ...
    "multi-agent": [
        ...
        Project("open-multi-agent", "open-multi-agent/open-multi-agent",
                "TypeScript orchestration **harness** where a coordinator plans the task DAG at runtime instead of a hand-authored graph (\"describe the goal, not the graph\"), running on Claude, GPT, Gemini, DeepSeek, or local models—pitched as a CrewAI/LangGraph alternative.",
                "slightly complex (runtime DAG planning, multi-provider)", oss="❓", labels=["javascript"]),
        Project("claude_codex_bridge", "SeemSeam/claude_codex_bridge",
                "tmux-based **harness** for running Codex, Claude, Gemini, Kimi, Qwen, Cursor, Copilot, Pi, and OpenCode side by side in one visible multi-agent workspace instead of switching terminals per agent.",
                "slightly complex (tmux multi-agent workspace)", oss="❓"),
    ],
    "plugins-mcp-cli": [
    ...
    "memory": [
        ...
        Project("MIRIX", "Mirix-AI/MIRIX",
                "Multi-agent personal-assistant **harness** that captures real-time on-screen activity and consolidates it into structured, queryable memory rather than raw logs—memory as the loop's core, not a bolt-on store.",
                "slightly complex (screen capture, structured memory)", oss="❓"),
    ],
    "evaluation": [
        ...
        Project("AssetOpsBench", "IBM/AssetOpsBench",
                "IBM's benchmark-and-orchestration harness for domain-specific Industry 4.0 agents: 460+ scenarios across five specialist agents (IoT, FMSR, TSFM, Work Order) plus MetaAgent/AgentHive orchestration blueprints over MCP.",
                "complex (multi-agent benchmark + orchestration blueprints)", oss="❓"),
    ],
    "observability": [
    ...
    "research-task": [
        ...
        Project("FAROS", "OpenNSWM-Lab/FAROS",
                "Blueprint-driven AutoResearch runtime that orchestrates the full research lifecycle as an agent **harness**: idea generation, experiments, paper writing, and peer review as one pipeline rather than separate tools.",
                "slightly complex (multi-stage research pipeline)", oss="❓"),
    ],
    "libraries-sdks": [
        ...
        Project("fast-agent", "evalstate/fast-agent",
                "Lean Python **harness** for building and evaluating agents with MCP, ACP, and A2A protocol support native to the loop rather than bolted on.",
                "mostly simple (MCP/ACP/A2A-native loop)", oss="❓", labels=["python"]),
...
META: dict[str, tuple[int, str, str]] = {
    ...
    "AntigmaLabs/ante": (1783, "https://github.com/AntigmaLabs/ante#readme", "Project README"),
...
KEEP_DESPITE_ARCHIVED: "set[str]" = {
    ...
    "RooCodeInc/Roo-Code",
    # 55k+ stars, one of the earliest and most widely-forked low-code
    # LLM/agent builders (drag-drop flows, agentic RAG); archived upstream
    # 2026-08-16. Still the reference point other low-code builders (e.g.
    # langflow-ai/langflow) get compared against. Historically important
    # enough to keep out of the Graveyard.
    "FlowiseAI/Flowise",
}
...
AXES: "dict[str, tuple[str, str]]" = {
    ...
    "superagent-ai/grok-cli": ("bounded", "resumable"),
    "AntigmaLabs/ante": ("bounded", "resumable"),
    "google-antigravity/antigravity-cli": ("bounded", "resumable"),
    "777genius/agent-teams-ai": ("n/a", "n/a"),
    "OpenCoworkAI/open-cowork": ("n/a", "n/a"),
    # coding-harness-configs
    ...
    "HKUDS/DeepCode": ("bounded", "resumable"),
    # frameworks
    ...
    "ag2ai/ag2": ("n/a", "n/a"),
    "Fosowl/agenticSeek": ("bounded", "retry"),
    # multi-agent
    ...
    "evalstate/fast-agent": ("bounded", "none"),
